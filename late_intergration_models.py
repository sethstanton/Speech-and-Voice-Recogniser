...
#         self.fc1 = nn.Linear(hidden_units * 221 * 8, output_size)
#
...
class AudioModel(nn.Module):
    def init(self, input_shape, hidden_units, output_size):
        super(AudioModel, self).init()

        # Assuming input_shape is (800, 442, 16)
        self.conv1 = nn.Conv2d(800, hidden_units, kernel_size=(3, 3), stride=1, padding=1)
        self.relu = nn.ReLU()
...
binary_data,binary_labels = load_binary_data()
dct_data,dct_labels = load_dct_data()
mfcc_data, mfcc_label = load_mfcc_data()
binary_X_train, binary_X_test, binary_y_train, binary_y_test = tts_data(binary_data,binary_labels)

dct_X_train, dct_X_test, dct_y_train, dct_y_test = tts_data(dct_data,dct_labels)

audio_X_train, audio_X_test, audio_y_train, audio_y_test = tts_data(mfcc_data,mfcc_label)

# print(torch.tensor.shape(audio_X_train))

audio_model = AudioModel(input_shape=(800, 442, 16), hidden_units=32, output_size=20)
...
audio_binary_visual_model = LateIntegrationModel(audio_input_shape=(442, 16, 1), visual_input_shape=(1, 110, 4), hidden_units=32, output_size=20)
audio_dct_visual_model = LateIntegrationModel(audio_input_shape=(442, 16, 1), visual_input_shape=(1, 110, 26), hidden_units=32, output_size=20)
